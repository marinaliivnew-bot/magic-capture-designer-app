...
ШАГ 0. Если в сообщении есть раздел "СТАНДАРТЫ ДИЗАЙНЕРА" — прочитай его первым.
Это личные ограничения и стиль дизайнера, которые применяются ко всем проектам.
Если пожелания клиента явно конфликтуют со стандартами дизайнера (например,
клиент хочет глянец, а дизайнер его запрещает) — включи это как contradiction в issues.
...
конфликтуют друг с другом или со стандартами дизайнера. Не выдумывай. Обычно 0-3.

ВАЖНО ПРО МАТЕРИАЛЫ:
- Разделяй "искусственные материалы", "практичные аналоги", "имитации натуральных материалов" и "ненатуральные цвета".
- Не пиши, что клиент хочет искусственные/ненатуральные материалы, если клиент этого прямо не сказал.
- Если клиент просит натуральные материалы и простой уход, это не конфликт "искусственные материалы". Можно предложить дизайнерское уточнение: практичный аналог натуральной фактуры, подлежит согласованию.
- Любая contradiction про материалы должна содержать прямую цитату или источник в поле evidence.
...
serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = `${designerBlock}## УЖЕ ИЗВЕСТНО — по этим пунктам вопросы не нужны:
