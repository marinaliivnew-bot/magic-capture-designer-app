...
import { getBrief, getProject, parseRoomsFromText, updateProject, upsertBrief } from "@/lib/api";
import { getRooms, saveRooms, uploadPlanFile } from "@/lib/rooms";
...
import { ArrowLeft, ChevronDown, FileText, FolderOpen, Image as ImageIcon, Loader2, Plus, Save, Scan, Sparkles, Upload, X } from "lucide-react";
...
const EditProject = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [project, projectRooms, brief] = await Promise.all([getProject(projectId), getRooms(projectId), getBrief(projectId)]);
        ...
        setUserRefs(normalizeUserRefs((brief as any)?.user_refs, "project"));
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:py-12">
        <div className="space-y-6">
          ...
          <div className="space-y-3 rounded-lg border border-border bg-card p-4">
            <div>
              <Label className="text-base font-semibold">А. Чертеж или план помещения</Label>
              <p className="mt-1 text-xs text-muted-foreground">Загрузите или замените план. После распознавания можно перенести помещения и габариты прямо в проект.</p>
            </div>
            ...
            {hasPlan ? (
              <>
                <div className="flex items-center gap-3 rounded-md border border-border bg-muted/50 p-3">
                  ...
                  <span className="flex-1 truncate text-sm text-foreground">{planFile?.name || "Загруженный план"}</span>
                  <Button type="button" variant="ghost" size="icon" className="h-8 w-8" onClick={removePlan}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>

                {!!planFile && !planAnalysis && planFile.type.startsWith("image/") && (
                ...
                {!!planFile && !planAnalysis && planFile.type === "application/pdf" && (
                  <Button type="button" variant="outline" className="w-full" onClick={handleAnalyzePlan} disabled={analyzingPlan}>
                    {analyzingPlan ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Scan className="mr-2 h-4 w-4" />}
          ...
          <div className="space-y-3 rounded-lg border border-border bg-card p-4">
            ...
            {!planAnalysis && <p className="text-xs text-muted-foreground">Чтобы перенести помещения из чертежа, сначала загрузите план и запустите его распознавание.</p>}
          </div>
          ...
          <div className="space-y-4 rounded-lg border border-border bg-card p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Здесь собираем референсы, переписку, голосовые расшифровки и любые сырые пожелания клиента. Это один из главных блоков для последующей структуризации brief-а.</p>
              </div>
            </div>
          ...
          <Button onClick={handleSave} disabled={saving} className="h-12 w-full text-base" size="lg">
